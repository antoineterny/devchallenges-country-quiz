...
class Question extends React.Component {
	...
	shuffleCountries(turn) {
		...
		const answerIndex = countries.findIndex(
			q => q.name === questions[turn].name
		)
		...
		const nextButton = (
			<button
				onClick={() => {
					...
					if (this.state.turn < this.props.nbQuestions)
						this.shuffleCountries(this.state.turn)
				}}
			>
				Next
			</button>
